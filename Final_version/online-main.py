...
def main_kpi(args, base_dir, data_path):
    ...
    ret_dir = osp.join(base_dir, "online_results")
    ...
    for name, group in data_df.sort_values(by=["KPI ID", "timestamp"], ascending=True).groupby("KPI ID"):
        ...
        # 由于本数据集仅覆盖约59.5分钟，不具备完整的周期性（如日周期），
        # 因此无法有效提取周期性模式，采用二阶平滑将导致计算误差或无效判断。
        # 为避免使用无意义的周期性特征，改为使用一阶平滑处理。
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='online', choices=['online', 'KPI'],
                        help="要处理的数据集: KPI 或 sock (默认: KPI)")
    parser.add_argument('--delay', type=int, default=7,
                        help="评估时的延迟点数 (默认: 7)")
    parser.add_argument('--q', type=float, default=0.1,
                        help="SPOT的风险系数 (默认: 0.05)")
    ...
    if flags.dataset == "online":
        base_dir = "./data/online/"
        data_path = osp.join(base_dir, "improved.csv")
        flags.delay = 7
        flags.q = 0.05
        print(f"\n处理KPI数据集: {data_path}")
