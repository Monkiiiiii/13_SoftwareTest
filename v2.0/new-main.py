...
def read_yahoo_data(path):
    ...
    dir_id = int(os.path.basename(os.path.dirname(path))[1])
...
def main_yahoo(args, data_dir):
    ...
    file_list = []
    for _id in [1, 2, 3, 4]:
        sub_dir = osp.join(data_dir, f"A{_id}Benchmark")
        file_list += glob(osp.join(sub_dir, "*.csv"))

    y_true, y_pred = [], []
    for _path in file_list:
        data_df, file_name, dir_id = read_yahoo_data(_path)

        print(f"\n处理Yahoo文件: {file_name} (目录ID: {dir_id})")
        value = data_df["value"].values
        ...
        if not args.train_len:
            ...
            print(f"- 默认训练数据长度: {train_len}/{len(value)}")
        else:
            train_len = args.train_len
            print(f"- 自定义训练数据长度: {train_len}/{len(value)}")
        ...
        alarms, kpi_threshold = detect(value, train_len, period, smoothing,
                                       args.s_w, args.p_w, args.half_d_w, args.q,
                                       args.estimator, kpi_id=file_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='KPI', choices=['KPI', 'Yahoo'],
                        help="要处理的数据集: KPI 或 Yahoo (默认: KPI)")
